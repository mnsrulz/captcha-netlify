 ...
 * @access public
 */
export const setImage = (TessModule: { FS: { writeFile: (arg0: string, arg1: Uint8Array) => void; }; }, 
    api: { SetImageFile: (arg0: number, arg1: number) => any; }, 
    image: Uint8Array, angle = 0) => {
  // Check for bmp magic numbers (42 and 4D in hex)
  const isBmp = (image[0] === 66 && image[1] === 77) || (image[1] === 66 && image[0] === 77);

  const exif = parseInt(image.slice(0, 500).join(' ').match(/1 18 0 3 0 0 0 1 0 (\d)/)?.[1] || '1', 10) || 1;
